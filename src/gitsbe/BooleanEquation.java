...
//		}
	...
	/**
	 * Build equation from Boolean expression. Currently expressions must be of the following type:
	...
	public BooleanEquation (String equation)
	{
		...
		// Trim spaces until only single spaces are present, which will be used as split points
		int length = 0 ;
		do {
...
//			equation += ")" ;
		...
		if ((activatingRegulators.size() > 0) && (inhibitoryRegulators.size() > 0))
			equation += link ;
...
//		BclxL *= ((NFKB) or STAT3) and  not (((BID) or GZMB) or DISC)
	...
	public void mutateRandomActivatoryOperator ()
	{
		...
		{
			int randomIndex = Gitsbe.randInt(0, operatorsActivatingRegulators.size()-1) ;
	...
	public void mutateRandomInhibitoryOperator ()
	{
		...
		{
			int randomIndex = Gitsbe.randInt(0, operatorsInhibitoryRegulators.size()-1) ;
	...
	public void shuffleRandomActivatingRegulatorPriority ()
	{
		...
		{
			int randomIndex = Gitsbe.randInt(0, activatingRegulators.size() - 2) ;
	...
	public void shuffleRandomInhibitoryRegulatorPriority ()
	{
		...
		{
			int randomIndex = Gitsbe.randInt(0, inhibitoryRegulators.size() - 2) ;
	...
	public SingleInteraction[] getSingleInteractions ()
	{
		...
		for (int i = 0; i < inhibitoryRegulators.size(); i++)
		{
			singleInteractions[i + activatingRegulators.size()] = new SingleInteraction (inhibitoryRegulators.get(i), "-|", target) ;
		}

...
public class Launcher {
	...
	private void setupInputAndRun(String[] args) {
		if (args.length == 0) {
			...
			System.out.println(
					"Usage: gitsbe <name project> <filename network> <filename training data> <filename config file>");
			System.out.println("\nExample Testrun (command line usage): ");
			
			String[] test_args = new String[] { "example_run_ags", "toy_ags_network.sif",
					"toy_ags_training_data.tab", "toy_ags_config.tab", "toy_ags_modeloutputs.tab" };
			System.out.println("java -cp /pathToBinFolderOfGitsbeProject gitsbe.Launcher " + test_args[0] + " " + test_args[1] + " " + test_args[2] + " " + test_args[3] + " " + test_args[4] + "\n\n");
		} else {
			...
			// Repeat process 100 times to obtain confidence intervals for each evolutionary
			// run - to be used for publication
			// for (int j = 0; j < 100; j++)
			// {
